(function () {
  ...
  Game.DIM_X = 600;
  Game.DIM_Y = 600;
  Game.NUM_ASTEROIDS = (5 + (5*Game.DIFFICULTY));
  Game.NUM_STARS = 30;
  ...
  Game.prototype.addAsteroids = function () {
    ...
    while (num > 0) {
      ...
      if (Asteroids.Util.distance(pos, [300,300]) < range) {
        continue;
      }
      this.asteroids.push(new Asteroids.Asteroid(pos, this, Game.DIFFICULTY));
      num--;
    };
  };
  ...
  Game.prototype.checkAsteroids = function () {
    if (this.asteroids.length === 0 && !this.shipDestroyed) {
      Game.DIFFICULTY += 1;
      this.ship.vel = [0,0];
      this.ship.pos = [300,300];
      setTimeout(function() { this.addAsteroids(); }.bind(this), 3000);
    }
  ...
  Game.prototype.gameOver = function () {
    ...
    ctx.fillText('GAME OVER', 230, 275);
    ctx.fillText('restart with "r"', 210, 325);
    this.restartable = true;
    // setTimeout(function() { ctx.fillText('Restart by pressing "R"', 300, 350);}, 3000);
