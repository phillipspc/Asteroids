(function () {
  ...
  Game.prototype.checkCollisions = function() {
    ...
    for (var i = 0; i < game.allAsteroids().length; i++) {
      if (game.ship.isCollidedWith(allAsteroids[i])) {
    ...
    for (var j = 0; j < game.bullets.length; j++) {
      ...
      if (!(game.subAsteroids.length === 0) &&
          !(game.bullets.length === 0)) {
        for (var i = 0; i < game.subAsteroids.length; i++) {
          if (game.subAsteroids[i].isCollidedWith(game.bullets[j])) {
            game.remove(game.subAsteroids[i]);
            game.remove(game.bullets[j]);
            game.points += 15;
            break;
          }
