(function () {
  ...
  Game.prototype.safePos = function() {
    ...
    while (Asteroids.Util.distance(pos, this.ship.pos) < range) {
    ...
    for (var j = 0; j < this.bullets.length; j++) {
      for (var i = 0; i < this.asteroids.length; i++) {
        if (this.asteroids[i].isCollidedWith(this.bullets[j])) {
          this.points += 25 + (25*Game.DIFFICULTY);
          this.updatePoints();
  ...
  Game.prototype.death = function () {
    ...
    this.asteroids.forEach( function(asteroid) {
      asteroid.pos = game.safePos();
    });
    this.subAsteroids.forEach( function(subAsteroid) {
      subAsteroid.pos = game.safePos();
    });
  }
